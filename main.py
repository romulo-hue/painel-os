        ...
        <div class="container">
            ...
            <div class="card">
                ...
                <form method="post" action="/painel/veiculos/adicionar">
                    <div class="grid">
                        <div>
                            <label>Placa</label>
                            <input name="placa" placeholder="Ex.: SBB2B33" required>
                        </div>
                        <div>
                            <label>Código de frota</label>
                            <input name="cd_veiculo" placeholder="Ex.: 12040" required>
                        </div>
                        <div>
                            <label>Filial da O.S.</label>
                            <input name="cd_filial" placeholder="Ex.: 1" required>
                        </div>
                        <div>
                            <label>Centro de custo</label>
                            <input name="cd_ccusto" placeholder="Ex.: 420119" required>
                        </div>
                    </div>
                    <div class="acoes">
            ...
            <div class="card">
                ...
                <form method="post" action="/painel/veiculos/importar-xlsx" enctype="multipart/form-data">
                    ...
                    <div class="acoes">
                        <button class="btn-green" type="submit">Importar XLSX</button>
                    </div>
                </form>
                <p class="hint">O arquivo precisa ter as colunas: <b>placa</b>, <b>cd_veiculo</b>, <b>cd_filial</b>, <b>cd_ccusto</b>.</p>
            </div>
                ...
                <form method="get" action="/painel/veiculos">
                    <div class="grid">
                        <div>
                            ...
                            <input name="busca" value="{busca}" placeholder="Digite uma placa">
                        </div>
                    </div>
                    <div class="acoes">
            ...
            <div class="card" style="padding:0; overflow:auto;">
                <table>
                    ...
                    <tbody>
                        {linhas if linhas else '<tr><td colspan="7">Nenhum veículo cadastrado</td></tr>'}
                    </tbody>
                </table>
            </div>
        </div>
...
async def adicionar_veiculo(
    ...
    return RedirectResponse(
        url=f"/painel/veiculos?msg={mensagem}",
        status_code=303,
    )
...
def excluir_veiculo(
    ...
    return RedirectResponse(
        url="/painel/veiculos?msg=Veículo excluído com sucesso",
        status_code=303,
    )


@app.post("/painel/veiculos/importar-xlsx")
async def importar_veiculos_xlsx(
    ...
    if not colunas_obrigatorias.issubset(set(df.columns)):
        raise HTTPException(
            status_code=400,
            detail="O XLSX precisa ter as colunas: placa, cd_veiculo, cd_filial, cd_ccusto",
        )
